...
test (`Add employee Delails using datadriven -${addemployees[EMPADD].empfullname}`, async ({page}) => {
    ...
    await page.locator("//button[contains(.,'Login')]").click();
        await page.locator("//input[@placeholder='First Name']").fill[EMPAdd].firstname);
        await page.locator("//input[@placeholder='Middle Name']").fill[EMPAdd].lastname);
        await page.locator("//input[@placeholder='Last Name']").fill[EMPAdd].middlename);
